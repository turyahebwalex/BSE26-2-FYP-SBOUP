...
shows `Failed to download remote update` or API calls hang:

1. **Phone and laptop must be on the same Wi-Fi.** Cellular data won't work
   in the default LAN setup.
2. **Test reachability from the phone's browser:**
   `http://<your-laptop-LAN-IP>:5000/api/health` (find the IP with
   `ip -4 addr show` on Linux / `ipconfig getifaddr en0` on macOS). If it
   times out, the Wi-Fi has client isolation (common on campus networks)
   — see step 4.
...
4. **Wi-Fi blocks device-to-device traffic? Use a tunnel.**
   - Mobile-side (Metro): `cd mobile && npm run start:tunnel`
   - Backend-side (so the phone can reach the API too): in another terminal
     `ssh -R 80:localhost:5000 serveo.net`, copy the
     `https://...serveousercontent.com` URL it prints, then put
     `EXPO_PUBLIC_API_URL=<that URL>/api` in `mobile/.env` and
